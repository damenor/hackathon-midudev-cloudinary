...
export const ReactCompareImage: React.FC<ReactCompareImageProps> = props => {
  ...
  // 0 to 1
  const [sliderPosition, setSliderPosition] = useState<number>(sliderPositionPercentage!)
  ...
  const rightImageRef = useRef<any>(null)
  const leftImageRef = useRef<any>(null)
  ...
  const styles: { [key: string]: React.CSSProperties } = {
    ...
    slider: {
      alignItems: 'center',
      cursor: (!hover && horizontal && 'ew-resize') || (!hover && !horizontal && 'ns-resize') as any,
      display: 'flex',
    ...
    rightLabel: {
      ...
      padding: '10px 20px',
      position: 'absolute',
      left: horizontal ? null : '50%' as any,
      right: horizontal ? '5%' : null as any,
      top: horizontal ? '50%' : null as any,
      bottom: horizontal ? null : '3%' as any,
      transform: horizontal ? 'translate(0,-50%)' : 'translate(-50%, 0)',
      transition: 'opacity 0.1s ease-out',
    },
    leftLabelContainer: {
  ...
  return (
    <>
      {skeleton && !allImagesLoaded && <div style={{ ...styles.container }}>{skeleton}</div>}
...
ReactCompareImage.defaultProps = defaultProps as any
